(() => {
  ...
  const THEME_META = Object.freeze({
    classic: { badge: 'Classic', background: 'linear-gradient(180deg,#2a4d7e,#070d15)', main: 'radial-gradient(circle at top,rgba(69,117,255,.12),transparent 35%),linear-gradient(180deg,#08111e,#040915)' },
    trains: { badge: 'Train station', background: 'linear-gradient(180deg,#4a5e3f,#09110f)', main: 'radial-gradient(circle at top,rgba(255,214,125,.14),transparent 35%),linear-gradient(180deg,#0e1610,#050908)' },
    space: { badge: 'Space', background: 'linear-gradient(180deg,#2e275a,#05060f)', main: 'radial-gradient(circle at top,rgba(185,156,255,.16),transparent 35%),linear-gradient(180deg,#0a0818,#03040a)' },
    animals: { badge: 'Animal world', background: 'linear-gradient(180deg,#5f4a35,#0c0d10)', main: 'radial-gradient(circle at top,rgba(142,240,177,.16),transparent 35%),linear-gradient(180deg,#10120f,#060706)' }
  });
  ...
  function currentThemeChoice(){
    return ui.themeChoice ? ui.themeChoice.value : 'classic';
  }
  ...
  function applyTheme(){
    ...
    const theme = THEME_META[currentThemeChoice()] || THEME_META.classic;
    if(root) root.style.background = theme.background;
    ...
    if(ui.touchInfo){
      ...
      ui.touchInfo.textContent = `${theme.badge} | ${reward.label}`;
    }
  }
  ...
  function applyBuildInfo(){
    ...
    if(ui.aboutBuildSummary){
      ui.aboutBuildSummary.textContent = `Running ${buildVersionLabel(APP_BUILD)} from ${APP_BUILD.source_file}. This experimental build adds EEG-ready markers, block pauses, family-based dashboards, regulation controls, and school-facing fidelity tracking on top of the developmental ChoiceTrain and CPAT pathways.`;
    }
  }
  ...
  function settingsSnapshot(){
    return {
      ...
      medicationContext: ui.medicationContext.value,
  ...
  function loadSettings(){
    ...
    if(ui.themeChoice) ui.themeChoice.value = settings.themeChoice || SESSION_CONTEXT_DEFAULTS.themeChoice;
    ...
    if(ui.medicationContext) ui.medicationContext.value = settings.medicationContext || '';
  ...
  function learnerContextSnapshot(){
    return {
      ...
      medication_context: ui.medicationContext.value.trim(),
  ...
  function beginQuestion(question, attemptIndex){
    ...
    if(ui.useTwoStage.checked || question.forcePromptStage){
      ...
      ui.promptSubtitle.textContent = question.ruleText || question.cueSummary || question.subtitle;
  ...
  function currentLearnerRecord(){
    ...
    learner.baseline = {
      ...
      medicationContext: ui.medicationContext.value.trim(),
  ...
  function applyLearnerFields(learner){
    ...
    ui.themeChoice.value = baseline.themeChoice || SESSION_CONTEXT_DEFAULTS.themeChoice;
    ...
    ui.medicationContext.value = baseline.medicationContext || '';
  ...
  async function exportEegMarkers(){
    ...
    const baseName = `${safeFilenamePart(currentPupilName())}_${APP_BUILD.version}_${localTimestampForFile()}_eeg`;
  ...
  function attachEventHandlers(){
    ...
    const settingsInputs = [
      ...
      'eegAutoPauseSec', 'researchCondition', 'eegExportFormat', 'faceName', 'notes', 'rewardChoice',
      ...
      'sleepQuality', 'capTolerance', 'breakCount', 'maintenanceStatus', 'generalisationStatus', 'medicationContext',
