(() => {
  const APP_BUILD = Object.freeze({
    ...
    source_file: 'choice_train_V1.3.1.html',
    source_script: 'choice_train_V1.3.1.js',
    based_on_branch: 'main',
    based_on_commit: '8db32b1',
  ...
  function buildQuestionFromKind(kind, index){
    ...
    if(kind === 'cpat-sustained'){
      ...
      if(variantId === 'signal-station'){
        ...
        const nonTargets = [
          shapeOption('cpat-sustained-red-warning', 'triangle', 'red', 'Red warning'),
          shapeOption('cpat-sustained-amber-signal', 'square', 'amber', 'Amber signal'),
          shapeOption('cpat-sustained-blue-marker', 'diamond', 'blue', 'Blue marker')
        ];
        const stimulus = targetPresent ? target : nonTargets[rand(nonTargets.length)];
        return baseQuestionVariant({
          ...baseQuestion,
          prompt: 'Tap only when the green signal appears.',
          cueSummary: targetPresent ? 'Go signal present: respond to the green signal.' : 'No-go signal present: wait without tapping.',
          target,
          stimulus,
          targetPresent,
          options: [stimulus],
          ...
          nonResponseIsCorrect: !targetPresent,
          choiceHint: targetPresent ? 'Go signal: tap now.' : 'Wait for the next signal.',
          expectedResponseType: targetPresent ? 'hit' : 'correct-rejection'
        }, variantId, 'Signal Station', 'Rule: Tap the green signal. Wait for every other signal.');
      }
      ...
      return baseQuestionVariant({
        ...
        requiresResponse: targetPresent,
        nonResponseIsCorrect: !targetPresent,
        choiceHint: targetPresent ? 'Star present: tap now.' : 'Target absent: wait.',
        expectedResponseType: targetPresent ? 'hit' : 'correct-rejection'
      }, variantId, 'Star Watch', 'Rule: Tap when the star appears. Wait on every other shape.');
    }
    ...
    if(kind === 'cpat-selective'){
      ...
      if(variantId === 'ticket-hunt'){
        ...
        return baseQuestionVariant({
          ...baseQuestion,
          prompt: 'Find the gold ticket.',
          cueSummary: 'Ignore the other travel symbols and select the gold ticket.',
          target,
          options,
          correctId: target.id,
          renderMode: 'grid',
          ...
          expectedResponseType: 'hit'
        }, variantId, 'Ticket Hunt', 'Rule: Ignore the other travel symbols. Select the gold ticket.');
      }
      ...
      return baseQuestionVariant({
        ...
        distractorCount: options.length - 1,
        expectedResponseType: 'hit'
      }, variantId, 'Blue Star Search', 'Rule: Ignore other colours and shapes. Select the blue star.');
    }
    ...
    if(kind === 'cpat-orienting'){
      ...
      if(variantId === 'platform-pointer'){
        ...
        return baseQuestionVariant({
          ...baseQuestion,
          prompt: 'Use the cue, then tap the train.',
          cueSummary: cueValidity ? `Valid cue: look ${cueSide} first.` : `Invalid cue: the cue points ${cueSide}, but the train will appear on the other side.`,
          cueSide,
          cueValidity,
          targetSide,
          target,
          ...
          distractorCount: 1,
          expectedResponseType: 'hit'
        }, variantId, 'Platform Pointer', 'Rule: Look where the cue points first, then tap the train even if the cue misleads.');
      }
      ...
      return baseQuestionVariant({
        ...
        distractorCount: 1,
        expectedResponseType: 'hit'
      }, variantId, 'Star Pointer', 'Rule: Look where the cue points first, then tap the star.');
    }
    ...
    if(kind === 'cpat-executive'){
      ...
      if(variantId === 'traffic-lights'){
        ...
        return baseQuestionVariant({
          ...baseQuestion,
          prompt: 'Wait for green, then tap Go.',
          cueSummary: 'The light starts red. Wait, then tap when it turns green.',
          promptPreviewOption: stopSignal,
          promptPreviewText: 'Red means wait',
          target,
          options: [target],
          correctId: target.id,
          renderMode: 'cpat-sustained',
          forcePromptStage: true,
          promptDelayMs: 900,
          requiresResponse: true,
          nonResponseIsCorrect: false,
          choiceHint: 'Green means Go.',
          expectedResponseType: 'hit'
        }, variantId, 'Traffic Lights', 'Rule: Wait through red. Tap when the light turns green.');
      }
      ...
      return baseQuestionVariant({
        ...
        distractorCount: 1,
        expectedResponseType: 'hit'
      }, variantId, 'Middle Arrow', 'Rule: Ignore the side arrows. Tap the direction of the middle arrow.');
    }
